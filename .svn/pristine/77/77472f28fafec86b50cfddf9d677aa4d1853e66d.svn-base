...
export default {
  /*
     ...
     * uploadData    obj            上传附件的附件参数 listType
     * */
  ...
  data () {
    ...
    return {
      ...
      data: data,
      fileSize: data.size,
      upUrl: config.urlPrefix + knowledgeApi.knowledgeFileUpload.path,
      ...
      upData: data.upData,
      fileData: {},
      fileType: 'doc,docx,xls,xlsx,ppt,ppts,pdf,mp4,webm,png,jpg,jpeg,bmp,gif'
    }
  },
  computed: {
    info () {
  ...
  methods: {
    ...
    converterTextList (list) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        arr[i].url = list[i].filePath || list[i].url
        arr[i].type = list[i].fileType || list[i].name.split('.')[list[i].name.split('.').length - 1]
      }
    ...
    beforeUpload (file) {
      ...
      this.upData.bsModule = 'knowledge'
