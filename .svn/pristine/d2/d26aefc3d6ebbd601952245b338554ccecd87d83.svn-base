  ...
  <div ref="content">
    <el-row>
      ...
      <el-col :span="3" style="float:right;">
        <el-input v-model="formInline.knowledgeContentName" placeholder="请输入关键性文字"/>
      </el-col>
    </el-row>
    <div id="nosocomialTable" ref="nosocomialTable">
      ...
      >
        ...
        <el-table-column prop="knowledgeContentName" label="内容名称">
          <template slot-scope="scope">
            <el-link type="primary" @click="viewContent(scope.row)">{{scope.row.knowledgeContentName}}</el-link>
          </template>
        </el-table-column>
        <el-table-column prop="remark" label="纠错内容">
          <el-tooltip placement="bottom">
            <template slot-scope="scope">
              <div slot="content">{{ scope.row.remark }}<br/>第二行信息</div>
            </template>
          </el-tooltip>
        </el-table-column>
      ...
      <Modal :mask-closable="false" close-on-click-modal="false" height="800" v-model="viewModal" title="对话框标题"  class-name="vertical-center-modal" :loading="loading"  :width="800">
        ...
        <el-descriptions class="margin-top" :column="1">
          ...
          <el-descriptions-item label="纠错内容">
            <template>
              <div style="max-height: 400px;overflow:auto;">
                <el-row>
                  <el-col :span="3" style="width: 100px;">错误类型:</el-col>
                  <el-col :span="21">{{showData.errorTypes}}</el-col>
                </el-row>
                <el-row>
                  <el-col :span="3" style="width: 100px;">错误描述</el-col>
                  <el-col :span="21">{{showData.remark}}</el-col>
                </el-row>
                <span v-if="srcList.length > 0" v-for="item  in srcList" :key="item">
                <el-image  style="width: 100px; height: 100px;padding: 5px;" :src="item" :preview-src-list="srcList"></el-image>
                </span>
              </div>
            </template>
      ...
      <Modal v-model="removeModal" :mask-closable="false" close-on-click-modal="false" width="500" title="对话框标题" class-name="vertical-center-modal">
        ...
        <remove v-if="removeModal" :delete-url="knowledgeApi.delContentError.path+ idStr" :operaility-data="operailityData" @remove="subCallback" @cancel="cancel"/>
        <div slot="footer" />
      </Modal>
    </div>
  </div>
...
export default {
  ...
  data () {
    return {
      ...
      showData: {},
      listTotal:0,
      removeId:{id: 'removeId',title: '举报删除'},
      viewId:{id: 'viewId',title: '举报查看'},
      removeModal:false,
      viewModal:false,
      queryQptions:{},
      multipleSelection:[],
  ...
  methods: {
    ...
    viewContent(row){
      ...
      this.showData={}
      this.showData=row
      const imagesList=row.images.split(",")
      if(imagesList!=null&&imagesList.length>0){
        this.srcList=imagesList
      }
    },
    showImage(row) {
