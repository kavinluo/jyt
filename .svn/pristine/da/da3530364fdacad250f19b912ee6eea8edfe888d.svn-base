  ...
  <div>
    ...
    <template v-if="saved">
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="分数：" prop="tmMark">
                <el-input v-model="item.tmMark" placeholder="请输入分数"  type="number" oninput="value=value.replace(/[^\d]/g,'')"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="6">
      ...
      <div style="margin-top: 20px;text-align: center">
        <el-button type="primary" class="blueBtn" @click="submit('continue')">保存并继续录入</el-button>
        <el-button type="primary" class="blueBtn" @click="submit('close')">保存并关闭录入</el-button>
      </div>
    </template>
...
let Util = null
export default {
  props: ['tmSelectNum', 'tmTypeObj', 'examTmTypeObj', 'currentTreeData', 'editData'], // 选项
  data () {
    return {
      tableData: [],
  ...
  methods: {
    ...
    // 添加小题
    addSubtitle () {
      if (this.tmDetailQuestionVO.length === 0) {
    ...
    submit (type) {
      ...
      console.log(obj)
      this.save(obj, type)
      this.tmDetailQuestionVO.splice(0, this.tmDetailQuestionVO.length)
