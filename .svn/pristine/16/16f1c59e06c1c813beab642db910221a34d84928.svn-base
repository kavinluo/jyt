...
export default {
  ...
  methods: {
    ...
    addNextScene () {
      ...
      console.log(everySceneTitle, 'everySceneTitleeverySceneTitle')
      this.everyScene.push(everySceneTitle)
    },
    // 删除
    removeNextScene (index) {
      if (this.everyScene.length === 1) {
        this.$message({
          message: '最后一幕无法删除',
          type: 'warning'
        })
      }else {
        this.everyScene.splice(index, 1)
      }
    },
    // 获取第几幕
    getAct (Index) {
    ...
    save (data, Item) {
      ...
      this.ajax({
        ajaxSuccess: (res) => {
          console.log(res)
          console.log(Object.keys(Item).length, Object.keys(Item).length === 0)
          if(Object.keys(Item).length === 0) {
            this.$emit('close')
          }else {
