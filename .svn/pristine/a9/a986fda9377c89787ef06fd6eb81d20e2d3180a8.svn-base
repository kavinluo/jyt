<template>
  <div :class="{'onlyUploadShow': uploadShow, 'noFile': uploadShow && !fileList.length }">
    ...
    >
      <div
        v-if="listType=='text'"
        style="width:200px;height:150px;text:align:center;line-height:150px;"
      >
        <p class="el-icon-plus avatar-uploader-icon" />
        <p>请上传视频</p>
      </div>
      <i
        v-show="!uploadShow"
      ...
      >
        ...
        >，且不超过{{ +fileSize | formatUploadSize }}</span>
      </div>-->
      <div
        v-if="data.message"
        v-show="!uploadShow"
...
export default {
  ...
  methods: {
    ...
    beforeUpload (file) {
      ...
      //   // 调用[限制图片尺寸]函数
      //   this.limitFileWH(4, 3, file).then((res) => {
      //     file.isFlag = res
      //     if (file.isFlag) {
      //       return resolve(true)
      //     } else {
    ...
    limitFileWH (E_width, E_height, file) {
      ...
      const isSize = new Promise(function (resolve, reject) {
        ...
        _this.$message.warning({
          message: '上传视频的尺寸比例应为' + E_width + '*' + E_height + '，当前上传视频的尺寸为：' + imgWidth + '*' + imgHight,
          btn: false
        })
        return false
      })
    ...
    onSuccess (response, file, fileList) {
      ...
      this.uploadList = fileList
      this.$emit('setUploadFiless', response)
    },
    ...
    onPreview (file) {
      ...
      this.$emit('setUploadFiless', file.response)
      if(this.show) {
    ...
    onRemove (file, fileList) {
      ...
      // if (this.listType === 'picture-card') {
      //   this.uploadList = fileList
      //   this.$emit('setUploadFiless', this.processorPictureList(fileList), fileList, this.callData || {})
      //   return
      // }

      // this.$emit('setUploadFiless', this.processorList(fileList), fileList, this.callData || {})
