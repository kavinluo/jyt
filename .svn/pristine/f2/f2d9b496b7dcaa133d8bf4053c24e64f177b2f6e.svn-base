  ...
  <div id="contentSettingContent"  ref="contentSettingContent">
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane v-for="(item,index) in levelList"   :key="item.name" :label="item.lable" :name="item.name" >
        <settingLeave v-if="isRouterAlive" @cancelAdd="cancel" :businessType="businessType" :businessId="businessId" :level="index+1" :ref="'settingLeave'+(index+1)"></settingLeave>
      </el-tab-pane>
...
<script>
/* eslint-disable */
import settingLeave from './settingLeave.vue'
let Util = null
export default {
  components: {settingLeave},
     ...
     submitData(){
        ...
        if(this.levelList.length>0)  {
            for(var i = 0;i < this.levelList.length;i++) {
                this.$refs['settingLeave'+(i+1)][0].submitData();
                //var dataTablelist = this.$refs['settingLeave'+(i+1)][0].allForm.dataTable1;
                //debugger
                // this.allForm.dataTable1.push(...dataTablelist);
            }
        }
     },
    //通知父组件关闭本组件
