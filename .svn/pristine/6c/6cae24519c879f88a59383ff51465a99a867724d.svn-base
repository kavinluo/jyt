<template>
  <div ref="content">
    <el-row style="margin-bottom:20px">
      ...
      <el-col align="left" style="width:70px;float:left;margin-left:25px;">
        <el-button class="blueBtn">
          从excel导入成绩
        </el-button>
      </el-col>
      <!-- <el-col align="left" style="width:70px;float:left;margin-left:80px;">
        <el-button class="blueBtn">
          查询
        </el-button>
      </el-col> -->
    ...
    <table border="1" height="60px">
      <tr>
        ...
        <td width="300px">
          {{paperPassmark}}/{{paperConversion100}}
        </td>
      </tr>
      <tr>
        <td width="100px" style="background-color:#f2f2f2">
        ...
        <td width="400px">
         {{operailityData.zgksBeginTime | filterTime }}~{{operailityData.zgksEndTime | filterTime }}
        </td>
        <td width="100px" style="background-color:#f2f2f2">
          有效时间范围:
        </td>
    ...
    <div id="nosocomialTable" ref="nosocomialTable">
      <el-table :data="tableData" style="width: 100%" border>
        ...
        <el-table-column prop="exceptionStatus" label="状态"  width="80">
        ...
        <el-table-column prop="idNumber" label="证件信息"/>
        ...
        <el-table-column prop="answerTime" label="考试时间">
          <template slot-scope="scope">
        ...
        <el-table-column prop="mark" label="考试成绩">
          <template slot-scope="scope">
            ...
            <span v-if="isClick" @click="clickCell(scope.row)">{{ scope.row.mark }}</span>
            <el-input v-else v-model="scope.row.mark"  @blur="blurFn(scope.row)"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="markUserName" label="判分人"/>
        <el-table-column prop="address" label="成绩来源" />
        <el-table-column prop="address" label="操作" width="350" align="center">
          <template slot-scope="scope">
            <el-link type="danger" @click="removeList(scope.row)">删除</el-link>
...
export default {
  ...
  created () {
    this.init()
    console.log(this.operailityData, 'operailityData')
  },
  beforeDestroy () {
  ...
  methods: {
    ...
    getPaperData () {
      this.ajax({
        ajaxSuccess: (res)=>{
          console.log(res)
          let data = res.data
          this.paperPassmark = data.paperInfo && data.paperInfo.paperPassmark
          this.paperConversion100 = data.paperInfo && data.paperInfo.paperConversion100
          console.log(this.paperPassmark)
          console.log(this.paperConversion100)
        },
        ajaxParams: {
          url: '/paper/examPaperInfoArrange/get/' + this.operailityData.arrangeId,
          method: 'get'
        }
      })
    },
    ...
    blurFn (row) {
      ...
      let opt = {
        ...
        ajaxParams: {
          url: api.modifyList.path + row.markId,
          method: 'put',
          jsonString: true,
          data: {
            id: row.markId,
    ...
    removeList (row) {
      let opt = {
        ...
        ajaxParams: {
          url: api.removeCode.path + row.markId,
          method: 'delete'
        }
      }
      this.ajax(opt)
