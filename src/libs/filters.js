...
//      '4': '教育处已确认'
  ...
  const filters = [ // 声明全局过滤器及回调函数
    { // 格式化时间 年月日
      ...
      call (val) {
        if (!val) return ''
        let arr = val.substring(0, 10).split('-')
        let h = val.substring(11, 13)
        return h >= 12
          ? arr[0] + '年' + arr[1] + '月' + arr[2] + '日 ' + '下午'
    ...
    { // 货币格式化
      ...
      call (value, n, str) { // n 保留小数点位数 | str 货币前缀 | 使用： 10 | money(3,'$') ===> $ 10.000
        ...
        if (!isNaN(value)) {
          ...
          var intSum = ret.substring(0, ret.indexOf('.')).
    ...
    { // 时间格式化
       ...
       */
      call (value, format) {
        return typeof date == 'string' ? parseDate(value, format) : formatDate(
          value, format)
      },
    },
    { // 用户角色字典
      ...
      call (value, v, getByCode) {
        if (!dictionary[getByCode]) {
          let opt = {
            ajaxSuccess: res => {
              let t = {}
              if (res.data instanceof Object && res.data.child instanceof
                Array) {
            ...
            ajaxParams: {
              url: '/passport/pc/dictionary/getByCode/' + getByCode,
            },
          }
          v.ajax(opt)
        }
    ...
    },
    {
      ...
      call (value, fileterName) {
        ...
        if (!value) {
          ...
          value.split(',').
            map(item => list.push(
              typeNames[fileterName][item.toLocaleUpperCase()]))
        }
        return list.join('、')
      },
    },
    ...
    },
    {
      ...
      call (second) {
        ...
        arr = [h, m, s]
        return arr.map(i => i.toString().padStart(2, '0')).join(':') ||
          '00:00:00'
      },
    },
    {
    ...
    },
    {
      ...
      call (val) {
        let str = [];
        (val && val.split(',') || []).map(
          item => str.push(typeNames.userType[item]))
        return str.join('、')
      },
    },
    {
    ...
    },
    {
      ...
      call (value, v) {
        if (dictionary.affairType && !dictionary.affairType._load ||
          ...
          let opt = {
            ...
            ajaxParams: {
              url: 'cstc/pc/affair/getAffairType',//url: 'cstc/pc/affair/getAffairType'
            },
          }
          dictionary.affairType = {}
