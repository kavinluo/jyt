...
<script>
import addCustom from './examCategory_add' // 自定义增加节点
import editCustom from './examCategory_add' // 自定义编辑节点
// 引入--访问前缀--组件
...
export default {
  ...
  methods: {
    ...
    // 新建
    adds () {
      if(this.sltedTreeData.type === 'ORG_TK') {
        this.errorMess('机构题库不能新建节点内容')
        return
      }
    ...
    /* --点击--删除--按钮--*/
    remove () {
      if(this.sltedTreeData.id === '-1' || this.sltedTreeData.parentId === 0) {
