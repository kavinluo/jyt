...
<template>
  <div>
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="知识点：" prop="tmKnowledge">
            <el-input
              v-model="formValidate.tmKnowledge"
              placeholder="请输入知识点"
            />
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="题目难度：" prop="tmDifficulty">
            <el-select
              v-model="formValidate.tmDifficulty"
              placeholder="请选择"
            >
              <el-option
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="教学要求：" prop="tmRequire">
            <el-select
              v-model="formValidate.tmRequire"
              placeholder="请选择"
            >
              <el-option
    ...
    <examTmType-a1
      ...
      :tm-type-id="formValidate.tmTypeId"
...
<script>
/* 当前组件必要引入*/
import examTmTypeA1 from './examTmType/examTmTypeA1' // A1  A2
...
export default {
  ...
  created () {
    if (this.rowData.tmType) {
      this.currentExamTmType = this.rowData.tmType
    }
    this.formValidate.treeId = this.currentTreeData.id
    this.formValidate.treePath = this.currentTreeData.path
    this.init()
  },
  mounted () {
  },
  methods: {
    ...
    tmTypeIChange (val) {
      ...
      if (this.examTmTypeObj[val].isGroup === 0) {
        this.formValidate.tmGroupType = 'BASE'
      }else if(this.examTmTypeObj[val].isGroup === 1 && val !== 7) {
        this.formValidate.tmGroupType = 'GROUP'
      }else if (val === 7) {
        this.formValidate.tmGroupType = 'BIG_GROUP'
      }
      this.formValidate.tmTypeId = val
      this.formValidate.tmType = this.examTmTypeObj[val].name
      // 设置小题题型候选项并默认选中
    ...
    getExamTmDetail () {
      this.ajax({
        ajaxSuccess: (res) => {
          ...
          this.tmTypeIChange(this.formValidate.tmTypeId)
        },
        ajaxParams: {
          url: '/tkmanage/examTmDetail/get/' + this.rowData.id,
          method: 'get'
        }
      })
    },
    ...
    submit (data, type, eleName) {
      ...
      this.formValidate.tmDetailQuestionVO = data
      this.formValidate.tmDetailQuestionVO.tmTypeId = this.tmTypeId
      this.formValidate.tmDetailQuestionVO.tmType = this.tmType
      this.formValidate.tmAnswerVOList = [{
      ...
      this.formValidate.tmDetailQuestionVO.tmMark = this.formValidate.tmMark
      this.formValidate.tmDetailQuestionVO.tmNo = 1
      this.formValidate.tmContentDescription = data.textTmContent || data.tmContent
      this.formValidate.tmContentDescription = this.formValidate.tmContentDescription.substring(0, 10)
      if(data.isEnclosure) {
      ...
      this.ajax({
        ...
        ajaxParams: {
          url: this.rowData.id ? '/tkmanage/examTmDetail/modify/' + this.rowData.id : '/tkmanage/examTmDetail/add',
          method: this.rowData.id ? 'put' : 'post',
          jsonString: true,
