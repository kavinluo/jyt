...
<template>
  <div>
    <div class="tmContent">主题干：</div>
    <el-row>
      <el-col :span="20">
        <el-input v-if="tmContentShowType" type="textarea" :rows="5" v-model="formValidate.tmContent" :disabled="isSaveThemeStem" placeholder="请输入"></el-input>
        <editor-bar v-else v-model="formValidate.tmContent" :isClear="formValidate.isClear"
                    @change="changeEditor($event, 'tmContent')" :idEdit="isSaveThemeStem ? 'show' : ''"></editor-bar>
      </el-col>
      <el-col :span="4" style="line-height: 35px" align="center">
        <el-button class="blueBtn" @click="saveThemeStem">保存主题干</el-button>
    ...
    <div v-if="!isSaveThemeStem">
      ...
      <span>
        <upload-file :uploadUrl="'/passport/infra/file/upload'" :showFileList="false" :length="1"
                     @setUploadFiles="setUploadFiles($event, formValidate, 'videoFileId')" :uploadFiles="formValidate.fileRespVO"
                     :tipShow="false">
        </upload-file>
      </span>
    </div>
    <template>
      <el-table :data="tabledatas" style="width: 100%">
        <el-table-column prop="textTmContent" label="题干内容" width="180">
    ...
    <template v-if="saved">
      <div v-for="(item, index) in tmDetailQuestionVO" :key="index">
        <el-form :model="item" ref="formValidate" :inline="true" class="el-form-item-search" label-width="110px" :rules="rules" onsubmit="return false">
          <el-row>
            <el-col :span="8">
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="题目类型：" prop="tmTypeId">
                <el-select v-model="item.tmTypeId" @change="tkvalueChange($event,item)" placeholder="请选择">
                  <el-option
            ...
            <el-col :span="8">
              <el-form-item label="选项数：">
                <el-select v-model="item.tmSelectNum" @change="tmSelectNumChange($event,item)" placeholder="请选择">
        ...
        <el-row v-for="(ite, ind) in item.tmSelectList" :key="ind" style="margin-top: 20px">
          <el-col :span="2" style="line-height: 100px">
            <el-radio v-model="item.tmAnswer" :label="selectContent[ind]">{{selectContent[ind]}}</el-radio>
          </el-col>
          <el-col :span="18">
            <el-input v-if="ite.showInnput" type="textarea" :rows="4" v-model="ite.selectContent" placeholder="请输入"></el-input>
            <editor-bar v-else v-model="ite.selectContent" :isClear="false"
                        @change="changeEditors($event, ite)" :idEdit="editorType"></editor-bar>
          </el-col>
          <el-col :span="4" style="line-height: 35px" align="center">
            <el-button class="blueBtn" @click="changeInputs(ite)">{{ite.showInnput ? '图文编辑' : '文本编辑'}}</el-button>
            <upload-file :uploadUrl="'/passport/infra/file/upload'" :showFileList="false" :length="1"
                         @setUploadFiles="setUploadFiles($event, ite, 'videoFileId' )" :uploadFiles="ite.fileRespVO"
                         :tipShow="false">
            </upload-file>
          </el-col>
        </el-row>
      ...
      <div style="margin-top: 10px;text-align: center">
        <el-button class="blueBtn" @click="submit">保存小题</el-button>
      </div>
    </template>
    <div style="margin-top: 20px;text-align: center">
      <el-button class="blueBtn" @click="submit('close')">提交</el-button>
    </div>
...
import {newTopic as rules} from '../rules'
// 当前组件引入全局的util
let Util = null
export default {
  props: ['tmSelectNum', 'tmTypeObj', 'currentTreeData', 'editData', 'examTmTypeObj'], // 选项
  data () {
    return {
      rules,
      tabledatas: [],
  ...
  methods: {
      ...
      this.ajax({
        ajaxSuccess: (res) => {
          ...
          if(type === 'close') {
            ...
            // this.getExamTmDetail(res.data)
            this.addId = res.data
            this.saved = false
          }
        },
        ajaxParams: {
          url: '/tkmanage/examTmDetail/add',
          method: 'post',
          jsonString: true,
          data: data
        }
    ...
    getExamTmDetail (id) {
      this.ajax({
        ...
        ajaxParams: {
          url: '/tkmanage/examTmDetail/get/' + id,
          method: 'get'
        }
      })
    },
