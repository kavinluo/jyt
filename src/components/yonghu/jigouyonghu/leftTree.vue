...
<script>
import addCustom from './mechanism_add.vue' // 自定义增加节点
import editCustom from './mechanism_add.vue' // 自定义编辑节点
// 引入--访问前缀--组件
...
export default {
  ...
  methods: {
    ...
    operationTreeData (type) {
      ...
      if (isSltNode) {
        ...
        if (type === 'remove') {
          ...
          this.deleteUrl = '/passport/dept/delete'
        }
        this.openModel(type)
      }
    },
    ...
    adds () {
      ...
      this.addModal = true
      console.log(this.operailityType)
