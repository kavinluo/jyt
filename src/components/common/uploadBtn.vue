...
export default {
  /*
     ...
     * showFileList  boolean        是否显示上传附件列表
     * tipShow       boolean        是否显示提示信息
     * btnText       string         上传按钮显示文字
     * */
  ...
  props: ['uploadUrl', 'downServer', 'downloadUrl', 'type', 'accept', 'show', 'unSize', 'drag', 'size', 'message', 'length',
    ...
    'params', 'unMultiple', 'callData', 'uploadData', 'showFileList', 'tipShow', 'btnText'
  ],
  ...
  data () {
    ...
    return {
      ...
      data: data,
      fileSize: data.size,
      upUrl: config.urlPrefix + (this.uploadUrl || '/process/pc/file/upload'),
  ...
  methods: {
    ...
    init () {
      if (typeof this.data.url !== 'undefined') {
        ...
        this.upUrl = config.urlPrefix + '/process/pc/file/upload/static'
      }
    ...
    // 获取上传的附件id
    getUploadFileId (res) {
      console.log(res.relativePathFile, 'res.relativePathFile')
      if(res.relativePathFile) {
